...
$url=inspection_create_queue('Checkin',[['type'=>'Checkin','tool_id'=>(int)$item['tool_id'],'transaction_id'=>(int)$item['transaction_id'],'checkout_item_id'=>(int)$item['checkout_item_id'],'employee_id'=>(int)$item['employee_id']]],'/mobile/return.php');
json_response(['success'=>true,'tool_name'=>$item['name'],'inspection_url'=>inspection_public_url($url)]);
