...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name === '') {
        ...
        try {
            ...
            audit_log('department_created', null, $name);
            flash('success', 'Department added.');
        } catch (PDOException $e) {
            flash('danger', 'That department already exists.');
        }
    }

    redirect('/employees/departments.php');
}
...
if (isset($_GET['toggle'])) {
    ...
    if ($id) {
        db()->prepare(
            'UPDATE departments SET active = IF(active = 1, 0, 1) WHERE id = ?'
        )->execute([$id]);
...
$rows = db()->query(
    'SELECT * FROM departments ORDER BY active DESC, name'
)->fetchAll();
